...
mixin ApiError {
  ...
  Future<ApiErrorType> parseApiErrorType(dynamic error, {BuildContext? context}) async{
    if (error is DioError && error.type == DioErrorType.response) {
      ApiErrorCode errorCode = ApiErrorCode.unknown;
      print( error.message);
      return ApiErrorType(code: errorCode, message: error.message);
    } else  if(error is TimeoutException){
      return ApiErrorType(code:ApiErrorCode.unknown, message: error.message ?? "UnKnown");
    }
    return ApiErrorType();
  }
}
